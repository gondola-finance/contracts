...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (currentChain == CHAIN_ID.AVATEST) {
    for (const token of TOKENS) {
      ...
      await execute(token, { from: deployer, log: true }, "approve", poolAddress, amount)
      amounts.push(amount)
    }
    console.log(amounts)
    await execute("GondolaUSDPool", { from: deployer, log: true }, "addLiquidity", amounts, 1, new Date().getTime())
  } else {
